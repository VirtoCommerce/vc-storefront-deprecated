        ...
        public ActionResult Articles(string blogName, string filterType, string criteria, int page, int pageSize)
        {
            ...
            if (blog != null)
            {
                ...
                if (!string.IsNullOrEmpty(filterType) && !string.IsNullOrEmpty(criteria))
                {
                    ...
                    {
                        query = query.Where(a => !string.IsNullOrEmpty(a.Category) && a.Category.Handelize().EqualsInvariant(criteria));
                    }
                    if (filterType.EqualsInvariant("tag"))
                    {
                        query = query.Where(a => a.Tags != null && a.Tags.Select(t => t.Handelize()).Contains(criteria, StringComparer.OrdinalIgnoreCase));
                    }
                }
